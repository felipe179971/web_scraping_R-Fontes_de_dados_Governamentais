...
tryCatch(
  {
    for(cjd in 1:paginas_cj_dados){
      ...
      link_cj_dados<-link_cj_dados[-c(which(link_cj_dados%in%c(procurar)))]
      link_cj_dados<-link_cj_dados[-c(which(link_cj_dados%>%stringr::str_detect("q[=][&]sort[=][&]page")))]
      for(p_cjd in 1:length(link_cj_dados)){
        ...
        link_dataset<-link_dataset[which(link_dataset%>%stringr::str_detect("/dataset/"))]
        link_dataset<-paste0("https://dev.dados.gov.br",link_dataset[-which(link_dataset%>%stringr::str_detect("plus[.]google|twitter[.]com|facebook[.]com"))])
